...
const fieldClass = "mt-2 min-h-12 w-full rounded-xl border border-border bg-brand-bg px-4 text-sm text-dark outline-none transition focus:border-secondary focus:ring-2 focus:ring-secondary/15";
...
export function ContactForm() {
  ...
  return (
    <section className="rounded-3xl bg-clay p-6 sm:p-8" aria-labelledby="contact-form-heading">
      <h2 id="contact-form-heading" className="text-3xl font-extrabold tracking-tight">How can we help?</h2>
      <p className="mt-2 text-sm text-muted">This frontend form simulates submission for now. Our customer team will be connected later.</p>

      <form onSubmit={submitForm} className="mt-7 grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-bold text-dark sm:col-span-2">Message<textarea name="message" required rows={5} className={`${fieldClass} resize-y py-3`} placeholder="Tell us how we can help" /></label>
        <div className="flex flex-col items-start gap-3 sm:col-span-2 sm:flex-row sm:items-center">
          <button type="submit" className="section-action bg-primary px-6 text-white hover:bg-primary-hover"><Icon name="arrow" className="size-4" /> Send message</button>
