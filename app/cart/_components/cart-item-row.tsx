...
export function CartItemRow({ item, compact = false }: { item: CartItem; compact?: boolean }) {
  ...
  if (compact) {
    return (
      <m.article initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 24 }} className="flex gap-3 py-3">
        <ProductImage item={item} size="size-[72px]" />
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <button type="button" onClick={() => removeItem(item.key)} className="text-xs font-semibold text-muted hover:text-primary">Remove</button>
          </div>
          <div className="mt-2 flex items-center justify-between gap-3">
            <QuantityControl item={item} onChange={updateQuantity} />
            <strong className="text-sm text-dark">{formatPrice(unitPrice * item.quantity)}</strong>
          </div>
        </div>
      </m.article>
    );
