...
export function ProductDetail({
  ...
  return (
    <main className="flex-1 pb-24 lg:pb-0">
      ...
      <section
        className="site-shell pb-5 sm:pb-6"
        ...
        <div className="grid overflow-hidden rounded-2xl border border-border bg-white shadow-sm lg:grid-cols-[1.02fr_.98fr]">
          <div className="relative h-[180px] overflow-hidden bg-stone-100 xs:h-[210px] sm:h-[260px] md:h-[300px] lg:h-auto lg:min-h-[480px]">
            ...
            <div className="absolute left-3 top-3 flex flex-wrap gap-1.5 sm:left-4 sm:top-4">
              ...
              {!!product.spicyLevel && (
                <Badge variant="neutral">🌶 Spicy</Badge>
              )}
            </div>
            <div className="absolute bottom-3 left-3 flex items-center gap-2 rounded-full bg-white/95 px-3 py-1.5 text-xs font-bold text-secondary shadow-md backdrop-blur sm:bottom-4 sm:left-4">
              <span
                className={`size-2 rounded-full ${product.available ? "bg-emerald-500" : "bg-stone-400"}`}
              />
          ...
          <div className="flex flex-col p-3 sm:p-4 lg:p-4">
            <div className="border-b border-border pb-3">
              ...
              <RatingStars
                ...
                className="mt-2"
              />
              <p className="mt-2 text-sm leading-5 text-muted sm:text-[15px]">
                {product.description}
              </p>
              <div className="mt-2 flex items-end gap-2">
                ...
                {product.oldPrice && !selectedVariant && (
                  <span className="pb-1 text-base font-medium text-muted line-through">
              ...
              <fieldset className="border-b border-border py-2.5">
                ...
                <div className="mt-1.5 grid gap-1.5 sm:grid-cols-2">
                  {product.variants.map((variant) => (
                    <label
                      key={variant.id}
                      className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border px-2.5 py-1.5 transition-colors ${variantId === variant.id ? "border-primary bg-primary-light" : "border-border hover:border-primary/40"}`}
                    >
                      <input
                        type="radio"
                        name="variant"
              ...
              <fieldset className="border-b border-border py-2.5">
                ...
                <div className="mt-1 space-y-0">
                  {product.addOns.map((addOn) => (
                    <label
                      key={addOn.id}
                      className="flex min-h-11 cursor-pointer items-center gap-2 rounded-lg px-0.5 py-1"
                    >
                      <input
                        type="checkbox"
            ...
            <div className="py-2.5">
              <label
                ...
                className="text-sm font-extrabold text-dark sm:text-base"
              >
            ...
            <div className="mt-auto hidden items-center gap-2.5 lg:flex">
              ...
              <button
                ...
                className={`flex h-12 flex-1 items-center justify-between rounded-lg px-4 text-sm font-extrabold text-white transition disabled:cursor-not-allowed disabled:bg-stone-300 ${added ? "bg-secondary" : "bg-primary hover:bg-primary-hover"}`}
              >
          ...
          <div className="site-shell">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
              {relatedProducts.map((item) => (
                <FoodProductCard key={item.id} product={item} compact />
              ))}
            </div>
          </div>
        </section>
      ...
      <div className="fixed inset-x-0 bottom-16 z-30 border-t border-border bg-white/95 px-3 py-2 shadow-[0_-8px_30px_rgba(29,37,34,.12)] backdrop-blur lg:hidden">
        <div className="mx-auto flex max-w-lg items-center gap-2">
          ...
          <button
            ...
            className={`flex h-12 flex-1 items-center justify-between rounded-xl px-4 text-sm font-extrabold text-white disabled:bg-stone-300 ${added ? "bg-secondary" : "bg-primary"}`}
          >
            <span>
              {added
                ? "View cart"
